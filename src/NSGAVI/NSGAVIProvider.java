...
import NSGAVI.NSGAVINondominatedSortingPopulation;
...
public class NSGAVIProvider extends AlgorithmProvider {

	@Override
/*	public Algorithm getAlgorithm(String name, Properties properties, Problem problem) {
		if (name.equalsIgnoreCase("NSGAVI")) {
...
*/	public Algorithm getAlgorithm(String name, Properties properties, Problem problem) {
		...
		try {
			if (name.equalsIgnoreCase("NSGAVI") ||
					name.equalsIgnoreCase("NSGA-VI") ||
					name.equalsIgnoreCase("NSGA6")) {
				return newNSGAVI(typedProperties, problem);
			} else {
				return null;
			}
	...
	private Algorithm newNSGAVI(TypedProperties properties, Problem problem) {
		
		int divisionsOuter = 4;
		int divisionsInner = 0;
		
		//System.out.println("Say hello from NSGAVI provider");
		...
		NSGAVIReferencePointNondominatedSortingPopulation population = new NSGAVIReferencePointNondominatedSortingPopulation(
		...
		/////////////////////NSGAVIReferencePointNondominatedSortingPopulation
		/*NSGAVINondominatedSortingPopulation population = 
				new NSGAVINondominatedSortingPopulation();// Create population
		...
		return new NSGAVI(problem, population, null, selection, variation,
				initialization);
	}
/*	private Algorithm newNSGAVI(TypedProperties properties, Problem problem) {
		...
		NSGAVINondominatedSortingPopulation population = 
				new NSGAVINondominatedSortingPopulation();
