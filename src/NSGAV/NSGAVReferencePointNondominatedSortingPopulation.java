...
//			NSGAIV.matrixPrint.printArray(Deltas.get(i));
	...
	*/
	...
	protected static Solution findMaxDistanceSolution (Population resultFilter){
		double[] Distance = new double[resultFilter.size()];
		for (int i=0; i<resultFilter.size();i++) {
	...
	protected static Solution findMinDistanceSolution (Population resultFilter){
		double[] Distance = new double[resultFilter.size()];
		for (int i=0; i<resultFilter.size();i++) {
			Distance[i] = (double)resultFilter.get(i).getAttribute(Distance_OBJECTIVES);
		}
	...
	public static Population addMinDistanceInGroup (Population resultFront, int newSize){
		...
		while(front.size()<newSize){
			//System.out.println("The size of new front is:" + front.size());
			Random rand = new Random();
			// nextInt is normally exclusive of the top value,
			// so add 1 to make it inclusive
			int randomNum = rand.nextInt(Grp_Max);
			if (Groups.get(randomNum).size()>0){
				Solution solution = findMinDistanceSolution (Groups.get(randomNum));
				Groups.get(randomNum).remove(solution);
				resultFront.remove(solution);
				front.add(solution);
			}
		}
		//System.out.println("After truncated with addMinDistance and Size:="+front.size()+"and newSize is:="+newSize);
		return front;
	}
	public static Population removeMinDistanceInGroup (Population resultFront, int newSize){
		for (int i = 0; i< resultFront.size();i++){
			resultFront.get(i).setAttribute(Group_OBJECTIVES,-1);
		}
		List<Population> Groups = new ArrayList<>();
	...
	public static Population removeMaxDistanceInGroup (Population resultFront, int newSize){
		for (int i = 0; i< resultFront.size();i++){
			resultFront.get(i).setAttribute(Group_OBJECTIVES,-1);
		}
		List<Population> Groups = new ArrayList<>();
		...
		while(resultFront.size()>newSize){
			...
			int randomNum = rand.nextInt(Grp_Max);
			if (Groups.get(randomNum).size()>0){
				Solution solution = findMaxDistanceSolution (Groups.get(randomNum));
				Groups.get(randomNum).remove(solution);
				resultFront.remove(solution);
			}
		}
	...
	public static Population removeMaxDistanceInGrid (Population resultFront, int newSize){

		while(resultFront.size()>newSize){
				resultFront.remove(findMaxDistanceSolution(resultFront));
		}
		//System.out.println("After truncated with removeMaxDistance and Size:="+resultFront.size()+"and newSize is:="+newSize);
		return resultFront;
	}
	public static Population removeByCrowdingDistance (Population resultFront, int newSize){
		while (resultFront.size()>newSize) {
			FastNondominatedSorting nondominatedSorting = new FastNondominatedSorting(comparator);
		...
		*/
		...
		if(previousFront.size()>0) {
			...
			*/	
			...
			//return addMinDistanceInGroup(currentFront,newSize);
		...
		/*
				...
				*/
				//System.out.println("The system test with Dung algorithms with resultFilter.size():"+ resultFilter.size() + ">newSize:"+newSize);
		//		resultFilter = currentFront;
				...
				//return removeByfindMax(currentFront, newSize);
				//return removeByCrowdingDistance(currentFront, newSize);				
		/*		return removeMaxDistance(resultFilter,newSize);
			}
...
//					currentFront.addAll(resultFilter);
	...
	public void updateMinMaxPoint(int sizeScale, Population population){
		...
		for (Solution solution : population) {
			...
			//double distance = MathArrays.distance(objectivesLow, objectivesBackup);
			double distance = MathArrays.distance(objectivesLow, objectives);
	...
	public void truncate(int size, Comparator<? super Solution> comparator) {
		//System.out.println("Hello from NSGAVReferencePoint.java");
		if (size() > size) {
			// remove all solutions past the last front
			sort(new RankComparator());
			//Population population = new Population();
			//for (int i = 0; i< size(); i++) population.get(i);
			...
			*/
			...
			// update the ideal point
			//System.out.println("hello from NSGAV Reference Point NondominatedSorting");
			updateIdealPoint();
			updateIdealMaxPoint();
			// translate objectives so the ideal point is at the origin
			translateByIdealPoint();
			...
			//int gridPoint = 10;//(int)((front.size())/(front.get(0).getNumberOfObjectives())+1);
			int gridPoint = (int)Math.ceil(Math.pow(tempGrid, 1.0 / (front.get(0).getNumberOfObjectives()-1)));
			//int gridPoint = (int)(tempGrid/front.get(0).getNumberOfObjectives()+1);
			//System.out.println("Need to remove :=" + tempGrid + "grid +"+ gridPoint +" _________ and front Size is "+ front.size() + "size is " + size + " size() is "+ size()+ " --------- and NUmber Objective is: " +front.get(0).getNumberOfObjectives());
			...
			*/
			//Population temp = addMinDistanceInGroup(front,N);
			//Population temp = removeMinDistanceInGroup(front,N);
			Population temp = removeMaxDistanceInGroup(front,N);
